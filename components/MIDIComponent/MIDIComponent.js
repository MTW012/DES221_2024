...
const newElement = (tag, id, clsName) => {
...
// handy function for checking whether a variable is a valid non-empty array
// const arrayOK = (x) => {
...
class CustomMIDI extends HTMLElement {
    ...
    constructor() {
      ...
      this.mainPanel = newElement('div', 'customMidiMainPanel', 'custom-midi main-panel horizontal-panel');
      shadow.appendChild(this.mainPanel);

      // Populate an input and output port table, with toggle buttons to connect/disconnect
      this.inputPortTable = newElement('div', 'customMidiInputPortTable', 'port-table vertical-panel');
      this.mainPanel.appendChild(this.inputPortTable);
      this.inputPortTableHeader = newElement('div', 'customMidiInputPortTableHeader', 'port-table-header');
      this.inputPortTableHeader.innerHTML = 'MIDI Inputs';
      this.inputPortTable.appendChild(this.inputPortTableHeader);

      this.outputPortTable = newElement('div', 'customMidiOutputPortTable', 'port-table vertical-panel');
      this.mainPanel.appendChild(this.outputPortTable);
      this.outputPortTableHeader = newElement('div', 'customMidiOutputPortTableHeader', 'port-table-header');
      ...
      if (this.midi) {
        if (this.midi.inputs) {
          this.midi.inputs.forEach((inputPort) => {
            ...
            const inputPortPanel = newElement('div', `inputPortPanel_${inputPort.id}`, 'port-panel horizontal-panel'); 
            this.inputPortTable.appendChild(inputPortPanel);
            // the name of the midi port
            const inputPortName = newElement('div', `customMidiInputPortName_${inputPort.id}`, 'port-name');
            ...
            const inputPortToggle = newElement('button', `inputPortToggle_${inputPort.id}`, 'port-toggle toggled-off');
        ...
        if (this.midi.outputs) {
          this.midi.outputs.forEach((outputPort) => {
            // a panel to hold the name of the port, and a toggle button for connecting
            const outputPortPanel = newElement('div', `outputPortPanel_${outputPort.id}`, 'port-panel horizontal-panel'); 
            this.outputPortTable.appendChild(outputPortPanel);
            // the name of the midi port
            const outputPortName = newElement('div', `customMidiOutputPortName_${outputPort.id}`, 'port-name');
            ...
            // toggle button for connecting
            const outputPortToggle = newElement('button', `outputPortToggle_${outputPort.id}`, 'port-toggle toggled-off');
